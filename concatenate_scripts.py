...
#         for root, dirs, files in os.walk(target_folder):
#             # Exclude certain directories (including the hardcoded path)
#             dirs[:] = [
#                 d for d in dirs
#                 if d not in ('__pycache__', 'venv', 'node_modules')
#                 and not os.path.normpath(os.path.join(root, d)) == HARDCODED_EXCLUDE_PATH
#             ]

#             for file in sorted(files):
#                 # Also skip if the *current* directory is the hardcoded exclude path
#                 if (
#                     '__pycache__' in root or
#                     'venv' in root or
#                     'node_modules' in root or
