...
def initialize_db():
    ...
    # --------------------------------------------------------------
    # NEW: create table if not exists for backtest_summary
    # so that queries to it won't fail with "no such table" error
    # --------------------------------------------------------------
    try:
        cur.execute('''
        ...
        logger.error(f"Error creating backtest_summary table: {str(e)}")
        raise
...
def serve_backtest_plot(filename):
    """
    Serves images generated by the Backtester in 'plots' folder.
    """
...
def run_backtest():
    """
    Runs the backtest, saves the plot in 'plots/' directory, and 
    returns a plot_url referencing /plots/<filename> so the front end can display it.
    """
    ...
    try:
        ...
        # If strategy parameters are passed, extract them here (empty by default)
        strategy_params = {}
        ...
        backtester = Backtester(
            ...
            # We do not pass `timeframe` to the constructor if the original backtester
            # code did not support it—this reverts to the prior behavior
        )
        backtester.run_backtest()

        # Retrieve the most recent backtest data from the DB
        conn = sqlite3.connect(os.path.join(project_root, 'data', 'backtesting_results.db'))
        cursor = conn.cursor()
        cursor.execute('''
            SELECT strategy_name, ticker, start_date, end_date, cagr, total_pct_change, std_dev, annual_vol, 
                   sharpe_ratio, sortino_ratio, max_drawdown, win_rate, num_trades, information_ratio, 
                   strategy_unique_id
        ...
        # Grab the final plot filename from the backtester
        plot_filename = getattr(backtester, 'plot_filename', None)
        plot_url = None
        if plot_filename:
            # Convert absolute path to a route-based URL: "/plots/filename.png"
            filename_only = os.path.basename(plot_filename)
            plot_url = f"/plots/{filename_only}"

        # Return JSON with both the metrics and the plot_url
        return jsonify({
            'success': True,
