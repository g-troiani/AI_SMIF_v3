...
class Backtester:
    ...
    def run_backtest(self, cash=100000.0, commission=0.0):
        ...
        try:
            ...
            ### 6) Generate the plot without a display (using Agg) and store the filename
            import matplotlib
            ...
            figs = cerebro.plot(
                ...
                bardown='red',
                volume=False,
                show=False  # Do not show an interactive window
            )

            # Typically, 'figs' is a list of lists, e.g. [[fig, axes], [fig, axes], ...].
            # We'll just take the first figure:
            fig = figs[0][0]

            # Optionally add a title
            fig.suptitle(
                f"Backtest Result - {self.strategy_name} on {self.ticker}",
                fontsize=16,
                fontweight='bold'
            )

            # Define the plots directory and ensure it exists
            import os
            from datetime import datetime
            plots_dir = os.path.join(self.project_root, 'plots')
            ...
            final_plot_filename = os.path.join(
                plots_dir,
                f"backtest_plot_{self.strategy_name}_{self.ticker}_{timestamp_str}.png"
            )
            ...
            # Store the final filename in self.plot_filename so external code
            # (e.g. Flask route) can return it
            self.plot_filename = final_plot_filename
            self.logger.info(f"Plot saved to {self.plot_filename}")
    ...
    def save_results(self, plot_filename=None):
        """
        Writes data to the new 'backtest_summary' table so the UI (which reads 
        from backtest_summary) can retrieve them. We remove all references to 
        the old 'backtest_results' to strictly move to backtest_summary.
        """
        ...
        # We'll store this DB path as before, but we only reference backtest_summary now.
        db_path = 'data/results/backtest_results.db'
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()

        # 1) Create the new 'backtest_summary' table if not exists.
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS backtest_summary (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                strategy_name TEXT,
        ...
        # 2) Gather backtest metrics from analyzers
        analyzer = self.results[0].analyzers
        returns_analysis = analyzer.returns.get_analysis()
        sharpe_analysis = analyzer.sharpe.get_analysis()
        drawdown_analysis = analyzer.drawdown.get_analysis()
        ...
        # 3) Insert the row into 'backtest_summary'
        cursor.execute('''
            INSERT INTO backtest_summary (
                strategy_name, strategy_params, ticker, start_date, end_date,
        ...
        # 4) Create/update the table 'backtest_plots' referencing backtest_summary
        cursor.execute('''
        ...
        # 5) If we have a plot filename, link it to the newly inserted row in backtest_summary
    ...
    def save_plot_filename(self, plot_filename):
        """
        Creates/updates the 'backtest_plots' table referencing the new 'backtest_summary' table
        (which is what snippet b) was already doing).
        """
        import os
        import sqlite3
        import logging

        with sqlite3.connect(os.path.join(self.project_root, 'data', 'backtesting_results.db')) as conn:
            cursor = conn.cursor()
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS backtest_plots (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
